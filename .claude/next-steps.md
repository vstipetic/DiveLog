...
```
Implement Phases 1 and 2 from .claude/next-steps.md:

PHASE 1: Fix Bugs
1. Fix FilterFunctions.py:
   - Remove duplicate dive_was_longer_than function
   - Change dive_data.basic_information → dive_data.basics throughout
   - Keep function signatures unchanged

PHASE 2: Pydantic Schemas & Statistics
1. Create Utilities/Schemas/ directory with:
   - ToolInputs.py (DepthFilterInput, DateRangeInput, etc.)
   - ToolOutputs.py (DiveSummary, FilterResult, StatisticsResult)
   - AgentModels.py (DepthProfile, DiveDetails, QueryResponse)

2. Implement Utilities/StatisticsFunctions.py with:
   - load_all_dives()
   - average_depth(), max_depth(), total_dive_time()
   - dive_count(), average_duration()
   - dives_by_year(), dives_by_month()
   - dives_by_location(), dives_by_buddy()
   - time_below_depth()
   
All functions should:
- Accept attrs-based Dive objects
- Return Pydantic StatisticsResult models
- Include comprehensive type hints
- Have docstrings

See .claude/existing-code.md for Dive/Gear class structure.
See .claude/claude.md for attrs vs Pydantic strategy.
DO NOT modify Dive/Gear classes or GarminDiveParser.
```